...
def get_actual_scm_mock(mocker):
    return mocker.patch('commandhelper.rules.scm_correction._get_actual_scm',
                        return_value=None)
...
@pytest.mark.parametrize('script, output, actual_scm', [
    ('git log', 'fatal: Not a git repository '
     ...
     'hg'),
    ('hg log', "abort: no repository found in '/home/sma-abyar/exp/commandhelper' "
               "(.hg not found)!",
     'git')])
...
@pytest.mark.parametrize('script, output, actual_scm', [
    ...
    ('hg log', "abort: no repository found in '/home/sma-abyar/exp/commandhelper' "
               "(.hg not found)!",
     None),
    ('not-scm log', "abort: no repository found in '/home/sma-abyar/exp/commandhelper' "
                    "(.hg not found)!",
     'git')])
